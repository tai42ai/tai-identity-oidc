...
def sign(key: RSAKey, claims: dict[str, Any], *, kid: str, alg: str = "RS256") -> str:
    ...
    ``kid`` is set independently of the signing key so a test can present a token
    whose header names a published kid while it was signed by a DIFFERENT key (the
    bad-signature case).
    """
...
class OidcServer:
    ...
    def sign(
        ...
        *,
        ...
        ``signing_kid`` selects the key that actually signs, defaulting to the
        header ``kid``. Passing a different ``signing_kid`` forges the
        bad-signature case: the header names a published key while a foreign key
...
class AcSettingsStub:
    ...
    The validate-only provider accepts this to satisfy the factory contract but
    reads nothing from it — its own config is the ``TAI_IDENTITY_OIDC_*`` env
    namespace — so a minimal structural stub is enough.
    """
