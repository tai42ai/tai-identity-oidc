...
``tai_identity_oidc`` is on the allowlist. The rule (see the README): the shipped
package imports
the OIDC/JWT stack (joserfc and cryptography, via tai-kit[jwt]), the HTTP client
(httpx), the shared platform substrate (tai-contract + tai-kit,
...
The runtime walk imports ``tai_identity_oidc`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai_identity_oidc`` means the assertion covers the SHIPPED package's
...
PACKAGE = "tai_identity_oidc"
ALLOWED_FIRST_PARTY = frozenset({PACKAGE, "tai_contract", "tai_kit"})

# Every third-party root the shipped ``tai_identity_oidc`` graph pulls in -- the declared runtime dependencies plus
...
def test_shipped_package_imports_only_allowlisted_roots() -> None:
    ...
    assert result.returncode == 0, (
        f"importing the shipped tai_identity_oidc graph failed:\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )

    offenders = [line for line in result.stdout.splitlines() if line]
    assert offenders == [], f"non-allowlisted roots in the tai_identity_oidc module graph: {offenders}"
...
def test_shipped_sources_name_only_allowlisted_roots() -> None:
    ...
    assert offenders == {}, f"non-allowlisted import roots in the tai_identity_oidc sources: {offenders}"
