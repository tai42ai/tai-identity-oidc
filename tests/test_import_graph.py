...
``tai42_identity_oidc`` is on the allowlist. The rule (see the README): the shipped
package imports
the OIDC/JWT stack (joserfc and cryptography, via tai42-kit[jwt]), the HTTP client
(httpx), the shared platform substrate (tai42-contract + tai42-kit,
...
The runtime walk imports ``tai42_identity_oidc`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai42_identity_oidc`` means the assertion covers the SHIPPED package's
...
# Every third-party root the shipped ``tai42_identity_oidc`` graph pulls in -- the declared runtime dependencies plus
